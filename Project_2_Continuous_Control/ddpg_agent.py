...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed):
        ...
        print('DDPG Agent hyperparameters:\n\t'
              ...
              .format(_batch_size, _buffer_size, _gamma, _lr_actor, _lr_critic, _tau, _noise_decay))

        # Actor Network
        self.actor_local = Actor(state_size, action_size,
                                 random_seed, [400, 300]).to(_device)
        self.actor_target = Actor(state_size, action_size,
                                  random_seed, [400, 300]).to(_device)
        self.actor_optimizer = optim.Adam(
            self.actor_local.parameters(), lr=_lr_actor)
        ...
        self.critic_optimizer = optim.Adam(
            self.critic_local.parameters(), lr=_lr_critic)

        # Initialize target networks weights with the local networks weights
    ...
    def act(self, state, noise=True):
        ...
        state = torch.from_numpy(state).float().to(
            _device)               # creates a tensor from a numpy array
    ...
    def soft_update(self, local_model, target_model, tau=_tau):
        ...
        ======
            ...
            target_model: PyTorch model (weight will be copied to)
        """
...
class ReplayBuffer:
    ...
    def sample(self):
        ...
        states = torch.from_numpy(np.vstack(
            [e.states for e in experiences if e is not None])).float().to(_device)
        actions = torch.from_numpy(np.vstack(
            [e.actions for e in experiences if e is not None])).float().to(_device)
        rewards = torch.from_numpy(np.vstack(
            [e.rewards for e in experiences if e is not None])).float().to(_device)
        next_states = torch.from_numpy(np.vstack(
            [e.next_states for e in experiences if e is not None])).float().to(_device)
