...
class Actor(nn.Module):
    ...
    def forward(self, x):
        ...
        # Pass the input through all the layers applying ReLU activation, except the last layer
        for layer in self.fc_layers[:-1]:
            x = F.relu(layer(x))

        # Pass the results through the output layer applying hyperbolic tangent function
...
class Critic(nn.Module):
    ...
    def __init__(self, state_size, action_size, seed, fc_layers=[128, 128]):
        ...
        # The second layer receives the first layer output + action
    ...
    def forward(self, state, action):
        ...
        for layer in self.fc_layers[1:-1]:
            x = F.relu(layer(x))
        # Pass the result through the output layer applying sigmoid activation
